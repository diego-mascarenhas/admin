...
<div id="contratar" class="firstTemplate"> <section class="container"> <div class="row"> <div
...
<div class="content-contacto margin-b-50">
	@guest
		...
		<a href="/login?redirect=contratar/plan/" class="button button-medium margin-auto bc-red-4 margin-t-40">Acceder</a>
		</p>
	@else
		<p class="margin-b-40">Hola {{ auth()->user()->name }}, ya se encuentra conectado a nuestro sistema y el plan quedará asociado a su
	empresa.</p>
	@endguest
	...
	<form action="{{ route('contratar.store') }}" method="post"> @csrf <input type="hidden" name="id" value="{{ $item->id }}">
		...
		@auth
			<input type="hidden" name="id_contacto" value="{{ auth()->user()->id }}"> <input type="hidden" name="nombre" value="{{ auth()->user()->name }}">
			<input type="hidden" name="email" value="{{ auth()->user()->email }}">
			<input type="hidden" name="razon_social" value="Usuario registrado previamente"> <input type="hidden" name="documento" value="Usuario registrado previamente">
		@else
		<div class="row">
