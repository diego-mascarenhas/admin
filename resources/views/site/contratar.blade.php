...
<div id="subheader">
	<header>
		<div class="subheader-banner" style="background-image:url('/assets/img/cabeceras/contacto.jpg');">
			<h1>Alta de plan</h1>
		...
		<div class="row">
			<div class="col col-md-8">
				...
				<div class="content-contacto margin-b-50">
					...
					@else
					<p class="margin-b-40">Hola {{ auth()->user()->name }}, ya se encuentra conectado a nuestro sistema
						y el plan quedará asociado a su empresa.</p>
					@endguest
					...
					<div class="col-md-12">
						...
						<p>
							<a href="{{ route('dashboard') }}"
								class="button button-medium margin-auto bc-red-4 margin-t-40">Mi cuenta</a>
						</p>
					</div>
					@endif
					...
					<form action="{{ route('contratar.store') }}" method="post"> @csrf <input type="hidden" name="id"
						...
						<div class="row">
							<div class="col-sm-6 form-group">
								<input type="text" name="razon_social" placeholder="Razón Social (*)"
									value="{{ old('razon_social') }}" class="form-control">
								@error('razon_social')
								<div class="text-danger small">{{ $message }}</div>
								@enderror
							</div>
							<div class="col-sm-6 form-group">
								<input type="text" name="documento" placeholder="NIF/CIF (*)"
						...
						<p class="margin-b-20">Ha seleccionado <strong>{{ $item->descripcion }}</strong>. A continuación
							se detallan las características</p>
						...
						<div class="form-group terminos">
							<input type="checkbox" name="terminos" value="1" class="margin-r-5" {{ old('terminos')
								? 'checked' : '' }}>
							Acepto todos los
