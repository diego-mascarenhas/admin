...
@section('content')
    <div id="subheader">
        <header>
            <div class="subheader-banner" style="background-image:url('/assets/img/cabeceras/web-hosting.jpg');">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            ...
                                            @elseif(isset($product->metadata->emails))
                                                {{ $product->metadata->emails }}
                                                Cuenta{{ $product->metadata->emails > 1 ? 's' : '' }} de
                                            ...
                                            {{ isset($product->metadata->backups)
                                                ? ($product->metadata->backups == 1
                                                    ? 'diarios'
                                                    : ($product->metadata->backups == 2
                                                        ? 'cada 48 hs'
                                                        : 'semanales'))
                                                : 'semanales' }}<br>
                                            ...
                                            @else
                                                <span style="text-decoration: line-through;">Dominio gratis por un
                                                    año</span><br>
                                            @endif
                                            ...
                                            @foreach ($plans_item->sortBy('unit_amount') as $plan)
                                                @if ($plan->currency === $configuredCurrency)
                                                    @php
                                                        ...
                                                        $currency = $currencySymbols[$plan->currency] ?? [
                                                    ...
                                                    @if ($plan->recurring->interval === 'month')
                                                        <p class="price" style="font-size: 1.4em;">
                                                            ...
                                                            <a href="#"
                                                                onclick="event.preventDefault(); document.getElementById('form-monthly-{{ $plan->id }}').submit();"
                                                    ...
                                                    @else
                                                        ...
                                                        <form action="/create-checkout-session" method="POST"
                                                            ...
                                                            <input type="hidden" name="price_id"
                                                                value="{{ $plan->id }}">
                                                            <button type="submit"
                                                                class="button button-medium margin-auto bc-red-4 margin-t-40">
                                                                contratar
                                                            </button>
                                                        </form>
                                                    @endif
